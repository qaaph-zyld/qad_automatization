...
def main():
    try:
        ...
        # Step 1: Read customer demand data from Excel
        logger.info("Step 1: Reading customer demand data from Excel")
        
        if args.excel_file:
            # Use specific Excel file if provided
            excel_file = args.excel_file
            logger.info(f"Using specified Excel file: {excel_file}")
            
            if not os.path.exists(excel_file):
                logger.error(f"Specified Excel file does not exist: {excel_file}")
                return 1
            ...
            logger.info(f"Looking for latest Excel file in directory: {excel_dir}")
            
            if not os.path.exists(excel_dir):
                logger.error(f"Excel directory does not exist: {excel_dir}")
                return 1
                
            excel_files = glob.glob(os.path.join(excel_dir, "*.xlsx"))
            
            if not excel_files:
                logger.error(f"No Excel files found in directory: {excel_dir}")
                return 1
                
            latest_file = max(excel_files, key=os.path.getmtime)
            logger.info(f"Latest Excel file: {latest_file}")
            
            demand_df = read_excel_data(latest_file)
        
        if demand_df is None:
            logger.error("Failed to read demand data. Exiting.")
            return
        
        # Step 2: Execute SQL query to get BOM data
        bom_df = execute_sql_query(args.sql_file, args.db_server, args.db_name)
        if bom_df is None:
            logger.error("Failed to get BOM data. Exiting.")
            return
        
        # Step 3: Analyze demand with BOM data
